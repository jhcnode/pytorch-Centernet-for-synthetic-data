...
class CenternetDetPatchSampler(data.Dataset):
	def get_offset(self,xmax,ymax,pw,ph,anns,ret_mask):
		def gen():
		...
		if(len(anns)==0):
			...
			range_mask=ret_mask[:xmax,:ymax]
			coords=np.argwhere(range_mask==False)
			coords=coords.tolist()
			while(True):
				idx=np.random.randint(0,len(coords),size=1)[0]
				offset_x,offset_y=coords[idx]
				bbox=[offset_x,offset_y,offset_x+pw,offset_y+ph]
				passed=True
				for ann in anns:
					ann_box=ann[:4]
					if(get_iou(bbox,ann_box)>0.6):
						passed=False
						break
				if(passed==True):
					break
	...
	def pacth_to_data(self,max_objs=5):
		...
		for patch_dir  in patch_dict.keys():
			...
			xmax=bw-pw
			ymax=bh-ph
			offset_x,offset_y=self.get_offset(xmax,ymax,pw,ph,anns,ret_mask)
			...
			ret_mask[offset_y:offset_y+ph,offset_x:offset_x+pw]=True
