...
class opts(object):
	def __init__(self):
		...
		self.parser.add_argument('--backbone', default=None, 
							 help='models backbone architecture. Currently tested')	
		...
		#demo
		self.parser.add_argument('--cam_res', default='1280,720',
		...
		self.parser.add_argument('--hide_data_time', action='store_false',
		...
		'test on test set')
		self.parser.add_argument('--subdivision', type=int, default=8,
							 help='subdivision')			
		...
		self.parser.add_argument('--rotate', type=float, default=0.5,
		help='when not using random crop'
	...
	def parse(self, args=''):
		...
		return self.parse_dict(opt)

	def parse_dict(self, opt):
		opt.phase="train"
		...
		if(opt.backbone is not None):
			backbone=''.join(x for x in opt.backbone if not x.isdigit())
			backbone=backbone.lower()
			if opt.head_conv == -1: # init default head_conv
				opt.head_conv = 256 if 'dla' in backbone else 64
			opt.pad = 127 if 'hourglass' in backbone else 31
			opt.num_stacks = 2 if backbone == 'hourglass' else 1	
	...
	def update_dataset_info_and_set_heads(self, opt, dataset):
		...
		if opt.task == 'ctdet' or opt.task == 'ctdetp':
